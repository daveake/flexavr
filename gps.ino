...
void ProcessUBX_NAV_PVT(unsigned char *Buffer)
{
  struct TUBlox
  {
    uint32_t GPSTime;
  ...
  if (UBlox->FixType > 0)
  {
    ...
    GPS.Hours = UBlox->Hours;
    GPS.Minutes = UBlox->Minutes;
    GPS.Seconds = UBlox->Seconds;
    GPS.Day = UBlox->Day;
    ...
    if (ShowGPS)
    {
      ...
      Serial.print(F("GPS="));
      sprintf(Temp, "%02d/%02d/%04d,", GPS.Day, GPS.Month, GPS.Year);
      Serial.print(Temp);
...
void ProcessUBX(unsigned char *Buffer)
{
...
//  Serial.print(" "); Serial.println(Buffer[3], HEX);
  ...
  {
    ProcessUBX_NAV_PVT(Buffer);
  }
}

void ProcessNMEA(unsigned char *Buffer)
{
...
void CheckGPS(void)
{
  ...
  static unsigned int UBXLength=0;
